...
const UserProfile = () => {
    ...
    const handleSave = () => {
        axios.put("http://localhost:5000/api/user", editedUser)
            .then((response) => {
                setUser(response.data);
                setIsEditing(false);
            ...
            <div className="border p-4 rounded-lg shadow flex items-center gap-4">
                ...
                {isEditing ? (
                    <div className="flex flex-col gap-2">
                        <input type="text" name="name" value={editedUser.name} onChange={handleInputChange} className="border rounded px-2 py-1" />
                        <input type="text" name="email" value={editedUser.email} onChange={handleInputChange} className="border rounded px-2 py-1" />
                        <input type="text" name="phone" value={editedUser.phone} onChange={handleInputChange} className="border rounded px-2 py-1" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {bookings.map((booking) => (
                    <div key={booking.id} className="border rounded-lg shadow p-4">
                        <img src={booking.image} alt={booking.hotelName} className="w-full h-32 object-cover rounded" />
